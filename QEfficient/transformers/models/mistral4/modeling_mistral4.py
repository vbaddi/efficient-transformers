...
def _qeff_mistral4_query_scale(position_ids: torch.Tensor, config) -> torch.Tensor:
    rope_parameters = getattr(config, "rope_parameters", {}) or {}
    beta = rope_parameters.get("llama_4_scaling_beta", 0.0) or 0.0
    ...
    return get_llama_4_attn_scale(safe_position_ids, beta, original_max_position_embeddings)


class QEffMistral4MoE(Mistral4MoE):
    ...
    def _decode_moe(
        ...
        gate_up_proj = self.all_gate_up_proj[topk_indices.flatten()]
        ...
        expert_out = torch.bmm(hidden, down_proj.transpose(1, 2))
...
class QEffMistral4Attention(Mistral4Attention):
    def __qeff_init__(self):
    ...
    def _mla_attention(
        ...
        query_states = query_states * _qeff_mistral4_query_scale(position_ids, self.config).to(query_states.dtype)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if compressed_kvs is not None:
            ...
            if attention_mask is not None:
                cache_kwargs["CCL"] = attention_mask.shape[-1]
            compressed_kvs.update_ckv(compressed_kv[..., : self.kv_lora_rank], self.layer_idx, cache_kwargs)
...
class QEffMistral4DecoderLayer(Mistral4DecoderLayer):
    def forward(
        self,
        hidden_states: torch.Tensor,
...
class QEffMistral4Model(Mistral4Model):
    def __qeff_init__(self):
        self.enable_mla = True
        self.mla_absorption_config = {"enable": True}
...
class QEffMistral4ForCausalLM(Mistral4ForCausalLM):
    def get_submodules_for_export(self) -> Type[nn.Module]:
        return {QEffMistral4DecoderLayer}
