...
class DequantizeLinearFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        return weight_unpacked.to(scale.dtype) * scale
...
class FP8DeQuantLinear(torch.nn.Module):
    ...
    def for_compressed_tensors_fp8_layer(
        ...
        if fp8_dequant_layer.weights_quantization_scheme.strategy == "tensor":
            fp8_dequant_layer.register_buffer("weight_scale", torch.zeros((1), dtype=torch.float16))
        elif fp8_dequant_layer.weights_quantization_scheme.strategy == "channel":
            fp8_dequant_layer.register_buffer("weight_scale", torch.zeros((out_features, 1), dtype=torch.float16))
        else:
            raise NotImplementedError(
        ...
        if not fp8_dequant_layer.input_activations_quantization_scheme.dynamic:
            if fp8_dequant_layer.input_activations_quantization_scheme.strategy == "tensor":
                fp8_dequant_layer.register_buffer("input_scale", torch.zeros((1), dtype=torch.float16))
            elif fp8_dequant_layer.input_activations_quant_scheme.strategy == "token":
                fp8_dequant_layer.register_buffer("input_scale", torch.zeros((1, in_features), dtype=torch.float16))
            else:
                raise NotImplementedError(
    ...
    def for_fp8_layer(cls, in_features, out_features, activation_quantization_strategy, bias):
        ...
        fp8_dequant_layer.register_buffer("weight_scale", torch.zeros((), dtype=torch.float16))

        if activation_quantization_strategy == "static":
            fp8_dequant_layer.register_buffer("input_scale", torch.zeros((), dtype=torch.float16))

        return fp8_dequant_layer

    def forward(self, x):
        dequantized_weights = DequantizeLinearFunc.apply(self.weight, self.weight_scale)
        # Only inference supported
        with torch.no_grad():
            out = torch.matmul(x.to(self.weight_scale.dtype), dequantized_weights.T)
...
class FP8BlockWiseDequantLinear(torch.nn.Module):
    ...
    def for_fp8_layer_with_blocksize(cls, in_features, out_features, weight_block_size, fmt, bias):
        ...
        fp8_dequant_layer.register_buffer(
            "weight_scale_inv",
            torch.empty(
                (out_features // weight_block_size[0], in_features // weight_block_size[1]), dtype=torch.float16
            ),
        )
        return fp8_dequant_layer
...
class FP8BlockWiseDequantQwen3VLMoeTextExperts(torch.nn.Module):
    def __init__(self, num_experts, moe_intermediate_size, hidden_size, act_fn, weights_block_size):
        ...
        self.register_buffer(
            ...
            torch.empty((self.num_experts, self.hidden_size // r, (2 * self.expert_dim) // c), dtype=torch.float16),
        )
        self.register_buffer(
            "down_proj_scale_inv",
            torch.empty((self.num_experts, self.expert_dim // r, self.hidden_size // c), dtype=torch.float16),
        )
        self.act_fn = act_fn
